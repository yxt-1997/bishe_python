...
for i in range(0,51,2):
    ...
    model=load_model('inception_4193.h5')#导入训练好模型的权重
    mark = ['.', 'o', '^', 's', 'D', '<', 'v', '*', 'd', 'h', '8']
    label = [0,1,2,3,4,5]
    m=i/2
    m=int(m);
    ...
    sio.savemat('E:/毕设/代码/407test/cnn/inception419_all3.mat', {'inception419_all3': p})
#各信号正确识别率
p=np.zeros([6,26])
data=[]
for i in range(0,51,2):
    ...
    for j in range(0, 6, 1):
        ...
        x_2=x_2.reshape(-1,1024,3)
        y_2 = np_utils.to_categorical(y_2, 6)
        model=load_model('inception_4193.h5')
        mark = ['.', 'o', '^', 's', 'D', '<', 'v', '*', 'd', 'h', '8']
        label = [0,1,2,3,4,5]
        m=i/2;
        m=int(m)
        ...
        y_pred=model.predict(x_2,batch_size=256)
        sio.savemat('E:/毕设/代码/407test/cnn/inception419_ever3.mat', {'inception419_ever3': p})